...
export default function WorkspaceView({ tab }: { tab: WorkspaceTab }) {
  ...
  const prevYears = useMemo(() => years.map(Number).filter(y => y < (Number(settings?.year) || 0)).sort((a, b) => b - a), [years, settings?.year]);
  ...
  return <section className="workspace-surface" aria-labelledby="workspace-page-title">
    <header className="workspace-page-header">
      <div><h1 id="workspace-page-title">{tabTitle(tab)}</h1><p>{tabSubtitle(tab)}</p></div>
      <span className="estimate-badge">No vinculante</span>
    </header>
    ...
    {tab === 'settings' && <section className="grid two">
      <Card title="Valores y reglas editables"><div className="form-grid">
        <Field label="Año comercial"><div><strong>{taxYear}</strong><small>Gestionado desde el contexto anual activo.</small></div></Field>
...
function tabTitle(tab: WorkspaceTab): string {
  ...
  } as Record<WorkspaceTab, string>)[tab];
}
