...
export function createMortgageRouter({ mortgageUseCases, annualRecordUseCases, context, resolveContext, readBody = readJsonBody, json = respond, apiError = respondError }) {
  return async function routeMortgages({ req, res, path, url }) {
    ...
    if (path === '/api/mortgages' && req.method === 'GET') {
      ...
      json(res, 200, await mortgageUseCases.listMortgageLoans(requestContext, filters));
      return true;
    }
    if (path === '/api/mortgages' && req.method === 'POST') {
      const body = await readBody(req);
      json(res, 201, await mortgageUseCases.createMortgageLoan(requestContext, body));
      return true;
    }
    const mortgageMatch = path.match(/^\/api\/mortgages\/([^/]+)$/);
    if (mortgageMatch && req.method === 'GET') {
      const r = await mortgageUseCases.getMortgageLoan(requestContext, mortgageMatch[1]);
      if (r) json(res, 200, r); else apiError(res, 404, 'not_found', 'Crédito no encontrado');
      return true;
    }
    if (mortgageMatch && req.method === 'PUT') {
      const updated = await mortgageUseCases.updateMortgageLoan(requestContext, mortgageMatch[1], await readBody(req));
      if (updated) json(res, 200, updated); else apiError(res, 404, 'not_found', 'Crédito no encontrado');
      return true;
    }
    if (mortgageMatch && req.method === 'DELETE') {
      if (await mortgageUseCases.deleteMortgageLoan(requestContext, mortgageMatch[1])) json(res, 204, {});
    ...
    if (annualRecordsMatch && req.method === 'GET') {
      ...
      json(res, 200, await annualRecordUseCases.listAnnualRecords(requestContext, annualRecordsMatch[1], filters));
      return true;
    }
    if (annualRecordsMatch && req.method === 'POST') {
      const body = await readBody(req);
      json(res, 201, await annualRecordUseCases.createAnnualRecord(requestContext, annualRecordsMatch[1], body));
      return true;
    }
    const annualRecordMatch = path.match(/^\/api\/mortgage-annual-records\/([^/]+)$/);
    if (annualRecordMatch && req.method === 'PUT') {
      const updated = await annualRecordUseCases.updateAnnualRecord(requestContext, annualRecordMatch[1], await readBody(req));
    ...
    if (annualRecordMatch && req.method === 'DELETE') {
      if (await annualRecordUseCases.deleteAnnualRecord(requestContext, annualRecordMatch[1])) json(res, 204, {});
